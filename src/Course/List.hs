...
length ::
  List a
  -> Int
length = foldRight (\_ count -> count + 1) 0
...
map ::
  ...
  -> List a
  -> List b
map f = foldRight (\a list -> (f a) :. list) Nil
...
filter ::
  ...
  -> List a
  -> List a
filter f = foldRight (\a -> if f a then (a :.) else (flip const) a) Nil
...
seqOptional (x :. xs) = 
  bindOptional (\a  ->
  bindOptional (\as -> Full (a :. as)) (seqOptional xs)) x
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 as = length (take 5 as) == 5
