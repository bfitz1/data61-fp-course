...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) ::
    ...
    -> StateT s k b
  (<*>) skf ska = StateT $ \s0 ->
    runStateT skf s0 >>= \(f, s1) ->
    runStateT ska s1 >>= \(a, s2) -> pure (f a, s2)
...
instance Monad k => Monad (StateT s k) where
  (=<<) ::
    ...
    -> StateT s k b
  (=<<) f ska = StateT $ \s0 ->
    runStateT ska s0 >>= \(a, s1) -> runStateT (f a) s1
...
runState' ::
  ...
  -> s
  -> (a, s)
runState' st s = let ExactlyOne x = runStateT st s in x
...
distinct' ::
  ...
  -> List a
distinct' as =
  let p = \a -> state' (\s -> (not (S.member a s), S.insert a s))
  in eval' (filtering p as) S.empty
...
distinctF ::
  ...
  -> Optional (List a)
distinctF as =
  let p = \a -> StateT (\s -> if a > 100 then Empty else Full (not (S.member a s), S.insert a s))
  in evalT (filtering p as) S.empty
...
instance Functor k => Functor (OptionalT k) where
  (<$>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  (<$>) f ota = OptionalT $ ((<$>) f) <$> runOptionalT ota
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) otf ota = OptionalT $ 
    runOptionalT otf >>= (\f ->
      case f of
        Empty -> pure Empty
        Full g -> (<$>) g <$> runOptionalT ota)

-- Interestingly, binding two separate times doesn't work!
-- It has to fail fast, I suppose?
...
instance Monad k => Monad (OptionalT k) where
  (=<<) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  (=<<) f ota = OptionalT $ runOptionalT ota >>= (\oa ->
    case oa of
      Empty -> pure Empty
...
instance Monad (Logger l) where
  (=<<) ::
    ...
    -> Logger l a
    -> Logger l b
  (=<<) f (Logger l1 a) = let (Logger l2 b) = f a in Logger (l1 ++ l2) b
...
distinctG as = 
  let logG a s
        | a > 100   = log1 (listh "aborting > 100: " ++ listh (show a)) Empty
        | even a    = log1 (listh "even number: " ++ listh (show a)) (Full s)
        | otherwise = pure (Full s)
      p a = StateT (\s -> OptionalT $ logG a (not (S.member a s), S.insert a s))
  in runOptionalT $ evalT (filtering p as) S.empty
